...
function getAIWorkerURL() {
  return window.AI_WORKER_URL || '';
}

function getApiKey() {
  // Kept for backward compatibility. API keys are no longer required in the browser.
  return '';
}

async function callAIWorker(payload) {
  const workerURL = getAIWorkerURL();
  if (!workerURL) throw new Error('AI Worker URL is not configured');
  ...
  if (!response.ok) {
    throw new Error(data.error?.message || data.error || data.message || `AI Worker Error ${response.status}`);
  }
  return data;
}
...
async function generateWithAI(type) {
  if (AI_STATE.isGenerating) return;
   /// Resume tracking
...
if(type === 'modern' && typeof incrementModern === 'function'){
  ...
  const purpose = document.getElementById('cvPurpose')?.value || 'professional';
  ...
  try {
    const model = document.getElementById('aiModel')?.value || 'openai/gpt-oss-120b';
    const tone = document.getElementById('aiTone')?.value || 'professional';
    const language = document.getElementById('aiLanguage')?.value || 'en';

    const prompt = buildPrompt(type, input, jobTitle, jobDesc, tone, language, purpose);
    updateAIProgressText('Contacting secure AI Worker...', 30);
    ...
    setTimeout(() => {
      ...
      if (type === 'all') {
        const completeResume = extractStructuredAIData(parsed, 'ats');
        if (completeResume) {
          applyAIToForm(completeResume);
        } else {
          showToast('The complete CV package was generated, but no editable resume data was returned.', 'warning');
        }
        return;
      }
    ...
    if (type === 'all') {
      const completeResume = extractStructuredAIData(fallback, 'ats');
      if (completeResume) applyAIToForm(completeResume);
    } else {
...
// ---- Build Prompts ----
function buildPrompt(type, input, jobTitle, jobDesc, tone, language, purpose = 'professional') {
  const currentData = buildTextFromState();
  const combinedInput = input || currentData;

  const purposeGuidance = {
    internship: 'The candidate may have no formal work experience. Do not invent employment. Prioritize education, coursework, projects, volunteering, extracurricular work, transferable skills, tools, and a learning mindset. Keep experience optional and low emphasis.',
    entry: 'The candidate is early-career. Prioritize skills, projects, education, internships, volunteering, and measurable academic or personal outcomes. Do not invent employers or dates.',
    'career-change': 'Show transferable skills and relevant projects prominently. Explain the transition positively and avoid overstating unrelated experience.',
    professional: 'Balance professional experience, achievements, skills, and education. Use concise accomplishment-focused bullets.',
    senior: 'Prioritize leadership scope, strategic impact, mentoring, ownership, and measurable business outcomes. Keep early-career details concise.'
  }[purpose] || 'Balance experience, skills, projects, and education appropriately.';

  const starterDraftGuidance = `If the user has supplied only a name and target position, create a useful, role-relevant starter draft for education, projects, and experience. Keep unknown institution, employer, dates, grades, and metrics clearly editable with labels such as [Add institution] or [Edit details]. For early-career users, use academic, project, volunteer, or training experience instead of pretending they held a job.`;

  const writingGuidance = `Write with a calm, specific, human style. Summaries should explain focus, strengths, and direction in 2-4 sentences. Experience should use concise action-led bullets. Projects must have a clear title, a short subtitle or context line, technologies, and a concrete description. Never use empty headings, repeated clichés, or unsupported claims.`;
...
CV purpose: ${purpose}
Purpose guidance: ${purposeGuidance}
${starterDraftGuidance}
${writingGuidance}
Never fabricate employers, degrees, dates, metrics, certifications, or achievements. Use empty arrays or clearly editable placeholders when information is missing. Return plain text only: never use Markdown bold, asterisks, or double-asterisk formatting.
Writing tone: ${tone}
Language: ${language}
`;
...

{
  "ats_resume": {
    ...
    "projects": [{"name": "Project title", "subtitle": "Short project context or role", "description": "...", "tech": "..."}],
    "certifications": [],
    "languages": []
  },
...
Create an ATS-optimized resume. Return ONLY valid JSON:
{
  "ats_resume": {
    ...
    "education": [{"degree": "...", "field": "...", "school": "...", "university": "", "startDate": "...", "endDate": "...", "gpa": "", "aggregate": ""}],
...
Write a compelling LinkedIn About/Summary section. Return ONLY valid JSON:
{
  ...
  return buildPrompt('all', input, jobTitle, jobDesc, tone, language, purpose);
}
...
function buildTextFromState() {
  ...
  d.education.forEach(e => {
    text += `\nEducation: ${e.degree} in ${e.field} from ${e.school}${e.university ? `, ${e.university}` : ''}${e.aggregate ? ` (${e.aggregate}%)` : ''}\n`;
  });

  d.projects.forEach(project => {
    text += `\nProject: ${project.name}${project.subtitle ? ` — ${project.subtitle}` : ''}${project.tech ? ` [${project.tech}]` : ''}\n`;
...
function parseAIResponse(content, type) {
  try {
    ...
    const parsed = sanitizeAsteriskText(JSON.parse(jsonMatch[0]));
    return {
      ats: parsed.ats_resume ? JSON.stringify(parsed.ats_resume, null, 2) : '',
      modern: sanitizeAsteriskText(parsed.modern_resume || ''),
      cover: sanitizeAsteriskText(parsed.cover_letter || ''),
      linkedin: sanitizeAsteriskText(parsed.linkedin_summary || ''),
      raw: parsed
    };
  } catch(e) {
    console.warn('JSON parse failed, using raw text:', e);
    const cleanContent = sanitizeAsteriskText(content);
    return {
      ats: type === 'ats' ? cleanContent : '',
      modern: type === 'modern' ? cleanContent : '',
      cover: type === 'cover' ? cleanContent : '',
      linkedin: type === 'linkedin' ? cleanContent : '',
      raw: null
    };
  }
}
...
function generateFallback(input, type) {
  ...
  const fallbackATS = {
    ...
    experience: [{
      id: 1, jobTitle: title, company: '', location: '',
...
// ---- AI Result Modal ----
function extractStructuredAIData(result, tab = 'ats') {
  if (result?.raw?.ats_resume) return result.raw.ats_resume;
  if (tab === 'ats' && result?.ats) {
    try { return JSON.parse(result.ats); }
    catch (_) { return parseTextToData(result.ats); }
  }
  return null;
}
...
function openAIResultModal(result, defaultTab = 'ats') {
  ...
  const autoApplied = defaultTab === 'ats' && Boolean(extractStructuredAIData(result, defaultTab));
  if (autoApplied) {
    try {
      applyAIToForm(extractStructuredAIData(result, defaultTab));
      showToast('AI resume applied to the builder automatically.', 'success');
    } catch (error) {
      console.warn('Automatic AI apply skipped:', error);
    }
  }
  ...
  if (modal) {
    ...
    const applyButton = document.getElementById('applyAIButton');
    if (applyButton) {
      applyButton.disabled = autoApplied;
      applyButton.innerHTML = autoApplied
        ? '<i class="fa-solid fa-check"></i> Applied Automatically'
        : '<i class="fa-solid fa-check"></i> Apply to Builder';
    }
  }
}
...
function showResultTab(tab) {
  ...
  content.textContent = sanitizeAsteriskText(text);
}
