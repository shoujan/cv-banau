...
function renderResume() {
  ...
  switch (STATE.ui.currentTemplate) {
    ...
    case 'nepali':   page.innerHTML = renderNepali(d); break;
    case 'nepali':   page.innerHTML = renderNepali(d); break;
    ...
    <div class="res-section">
      ...
      ${d.projects.map(p => `
        <div class="exp-item">
          ...
          ${p.subtitle ? `<div class="exp-company">${esc(p.subtitle)}</div>` : ''}${p.tech ? `<div class="exp-company">Technologies: ${esc(p.tech)}</div>` : ''}
      ...
      <div class="mod-section">
        ...
        ${d.projects.map(p => `
          <div class="mod-exp">
            ...
            ${p.subtitle ? `<div class="mod-exp-company">${esc(p.subtitle)}</div>` : ''}${p.tech ? `<div class="mod-exp-company">${esc(p.tech)}</div>` : ''}
...
// ============================================
// 6. NEPALI STYLE TEMPLATE
// Traditional Nepal CV structure based on the supplied reference image.
// ============================================
function renderNepali(d) {
  const p = d.personal;
  const fullName = [p.firstName, p.lastName].filter(Boolean).join(' ') || 'Your Name';
  const mobile = [p.phone, p.location].filter(Boolean).join(' / ');
  const languageText = d.languages.map(l => [l.language, l.proficiency].filter(Boolean).join(' - ')).filter(Boolean).join(', ');
  const address = p.correspondenceAddress || p.location;
  const photo = p.photo ? `<img class="np-photo" src="${esc(p.photo)}" alt="Profile photo of ${esc(fullName)}">` : `<div class="np-photo np-photo-placeholder">${esc(fullName.split(/\s+/).map(n => n[0]).join('').slice(0,2).toUpperCase())}</div>`;
  const eduRows = d.education.length ? d.education.map(e => `
    <tr><td>${esc([e.degree, e.field].filter(Boolean).join(' in '))}</td><td>${esc(e.school || '')}</td><td>${esc(e.endDate || e.startDate || '')}</td><td>${esc(e.university || '')}</td><td>${esc(e.aggregate || e.gpa || '')}</td></tr>`).join('') : `<tr><td colspan="5" class="np-empty-cell">Add education details in Manual Build</td></tr>`;
  const skills = d.skills.filter(Boolean);
  return `<div class="resume-nepali">
    <div class="np-header"><div><div class="np-name">${esc(fullName)}</div>${p.professionalTitle ? `<div class="np-title">${esc(p.professionalTitle)}</div>` : ''}</div>${photo}</div>
    <div class="np-section-title">Personal Details:</div>
    <div class="np-details">
      ${mobile ? `<div><b>Mobile:</b> ${esc(mobile)}</div>` : ''}
      ${p.email ? `<div><b>Email:</b> ${esc(p.email)}</div>` : ''}
      ${p.dob ? `<div><b>DOB:</b> ${esc(p.dob)}</div>` : ''}
      ${address ? `<div><b>Address:</b> ${esc(address)}</div>` : ''}
      ${p.permanentAddress ? `<div><b>Permanent Address:</b> ${esc(p.permanentAddress)}</div>` : ''}
      ${languageText ? `<div><b>Language:</b> ${esc(languageText)}</div>` : ''}
      ${p.maritalStatus ? `<div><b>Marital status:</b> ${esc(p.maritalStatus)}</div>` : ''}
    </div>
    ${d.summary ? `<div class="np-section-title">Career Objectives:</div><div class="np-paragraph">${esc(d.summary)}</div>` : ''}
    <div class="np-section-title">Educational Background:</div>
    <div class="np-table-wrap"><table class="np-education"><thead><tr><th>Qualification</th><th>Institution &amp;<br>Location</th><th>Completion<br>Year</th><th>University</th><th>Aggregate<br>%</th></tr></thead><tbody>${eduRows}</tbody></table></div>
    ${skills.length ? `<div class="np-section-title">Skills &amp; Abilities:</div><ul class="np-skills">${skills.map(skill => `<li>${esc(skill)}</li>`).join('')}</ul>` : ''}
    <div class="np-section-title">Experience:</div>
    <div class="np-experience">${d.experience.length ? d.experience.map(e => `<div class="np-experience-item"><b>${esc(e.jobTitle || '')}</b>${e.company ? ` — ${esc(e.company)}` : ''}<span>${esc([e.startDate, e.current ? 'Present' : e.endDate].filter(Boolean).join(' - '))}</span>${e.description ? `<p>${esc(e.description)}</p>` : ''}</div>`).join('') : '<div class="np-empty-note">No formal experience yet — add internships, projects, volunteering, or training in Manual Build.</div>'}</div>
    ${d.projects.length ? `<div class="np-section-title">Projects:</div><div class="np-projects">${d.projects.map(project => `<div class="np-project-item"><div class="np-project-title">${esc(project.name || 'Project title')}</div>${project.subtitle ? `<div class="np-project-subtitle">${esc(project.subtitle)}</div>` : ''}${project.tech ? `<div class="np-project-tech">${esc(project.tech)}</div>` : ''}${project.description ? `<p>${esc(project.description)}</p>` : ''}</div>`).join('')}</div>` : ''}
    ${d.certifications.length ? `<div class="np-section-title">Certifications:</div><ul class="np-skills">${d.certifications.map(c => `<li>${esc([c.name, c.issuer].filter(Boolean).join(' — '))}</li>`).join('')}</ul>` : ''}
  </div>`;
}
...
function getResumeAsText() {
  ...
  if (d.projects.length) {
    ...
    d.projects.forEach(pr => {
      text += `${pr.name}${pr.subtitle ? ' — ' + pr.subtitle : ''}${pr.tech ? ' | ' + pr.tech : ''}\n`;
