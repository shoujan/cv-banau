...
const STATE = {
  data: {
    personal: {
      ...
      website: '', linkedin: '', github: '', photo: '', dob: '', maritalStatus: '', correspondenceAddress: '', permanentAddress: ''
    },
    summary: '',
    experience: [],
    education: [],
...
function loadState() {
  try {
    ...
    // Migrate any previously saved deprecated model to the current Groq replacement.
    if (!STATE.settings.aiModel || STATE.settings.aiModel === 'llama-3.3-70b-versatile') {
      STATE.settings.aiModel = 'openai/gpt-oss-120b';
    }
...
function saveApiKey() {
  ...
  if (key) {
    ...
    setApiStatus('idle', 'AI ready via secure Worker');
  }
}
...
function collectFormData() {
  STATE.data.personal = {
    ...
    dob: STATE.data.personal.dob || '',
    maritalStatus: STATE.data.personal.maritalStatus || '',
    correspondenceAddress: STATE.data.personal.correspondenceAddress || '',
    permanentAddress: STATE.data.personal.permanentAddress || ''
  };
...
function addEducation(data = null) {
  ...
  const edu = data || { id, degree: '', school: '', field: '', university: '', startDate: '', endDate: '', gpa: '', aggregate: '' };
  ...
  STATE.data.education.forEach(edu => {
    ...
    div.innerHTML = `
      ...
      <div class="entry-body">
        ...
        <div class="form-row-2">
          <div class="floating-field">
            <input type="text" value="${esc(edu.university)}" placeholder=" " oninput="updateEdu(${edu.id}, 'university', this.value)">
            <label>University / Board (optional)</label>
          </div>
          <div class="floating-field">
            <input type="text" value="${esc(edu.aggregate)}" placeholder=" " oninput="updateEdu(${edu.id}, 'aggregate', this.value)">
...
function addProject(data = null) {
  ...
  const proj = data || { id, name: '', subtitle: '', description: '', tech: '', url: '', startDate: '', endDate: '' };
  ...
  STATE.data.projects.forEach(proj => {
    ...
    div.innerHTML = `
      ...
      <div class="entry-body">
        ...
        <div class="floating-field">
          <input type="text" value="${esc(proj.subtitle)}" placeholder=" " oninput="updateProj(${proj.id}, 'subtitle', this.value)">
          <label>Project Subtitle / Role</label>
        </div>
        <div class="floating-field">
...
function handlePhotoUpload(event) {
  ...
  if (file.size > 2 * 1024 * 1024) {
    showToast('Photo is too large. Maximum size is 2MB.', 'error');
    event.target.value = '';
    return;
  }
  const reader = new FileReader();
  reader.onload = (e) => {
    ...
    renderPhotoPreview(STATE.data.personal.photo);
    saveState();
    renderResume();
    showToast('Profile photo added.', 'success', 1800);
  };
...
function renderPhotoPreview(photo) {
  ...
  if (photo) {
    ...
    preview.innerHTML = `<img src="${esc(photo)}" alt="Selected profile photo">`;
...
function clearAllData() {
  ...
  STATE.data = {
    personal: { firstName: '', lastName: '', professionalTitle: '', email: '', phone: '', location: '', website: '', linkedin: '', github: '', photo: '', dob: '', maritalStatus: '', correspondenceAddress: '', permanentAddress: '' },
  ...
  if (typeof persistCurrentToProfile === 'function') persistCurrentToProfile();
...
function detectUniversityAndAggregate(text, entry) {
  const source = `${text || ''} ${entry?.school || ''} ${entry?.degree || ''} ${entry?.university || ''}`;
  const universityPatterns = [
    ['Tribhuvan University', /\b(?:TU|Tribhuvan University|IOE|Institute of Engineering)\b/i],
    ['Pokhara University', /\bPokhara University\b/i],
    ['Kathmandu University', /\bKathmandu University\b/i],
    ['Purbanchal University', /\bPurbanchal University\b/i],
    ['Mid-Western University', /\b(?:Mid[- ]?Western|MWU)\b/i],
    ['Far Western University', /\b(?:Far[- ]?Western|FWU)\b/i],
    ['NEB / HSEB', /\b(?:NEB|HSEB)\b/i]
  ];
  const university = entry?.university || universityPatterns.find(([, pattern]) => pattern.test(source))?.[0] || '';
  const aggregateMatch = source.match(/(?:aggregate|percentage|percent|gpa|grade)\s*[:=\-]?\s*([0-9]{1,3}(?:\.[0-9]+)?)\s*%?/i) || source.match(/\b([0-9]{2}(?:\.[0-9]+)?)\s*%/);
  const aggregate = entry?.aggregate || entry?.gpa || (aggregateMatch ? aggregateMatch[1] : '');
  return { university, aggregate };
}

// ---- AI text cleanup ----
// AI output is displayed as plain resume text, so Markdown asterisks must never leak into the UI.
function sanitizeAsteriskText(value) {
  if (typeof value === 'string') return value.replace(/\*/g, '');
  if (Array.isArray(value)) return value.map(sanitizeAsteriskText);
  if (value && typeof value === 'object') {
    return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, sanitizeAsteriskText(item)]));
  }
  return value;
}
...
function applyAIToForm(parsedData) {
  ...
  parsedData = sanitizeAsteriskText(parsedData);
  ...
  if (parsedData.education && Array.isArray(parsedData.education)) {
    const sourceText = document.getElementById('aiInput')?.value || '';
    STATE.data.education = parsedData.education.map((e) => ({ ...e, ...detectUniversityAndAggregate(sourceText, e), id: ++eduIdCounter }));
  }
